...
class RiskManager:
    ...
    - ONE position per market (no duplicates)
    ...
    async def evaluate(
        ...
        # STRICT: Only 1 position per market — no duplicates
        existing = [t for t in self.paper_engine.open_trades if t.market_id == signal.market_id]
        if existing:
            logger.info(f"Trade rejected (already have position in {signal.market_id})")
            return None
        ...
        loss_pct = (signal.entry_price - signal.stop_loss_price) / signal.entry_price
        if loss_pct > 0:
